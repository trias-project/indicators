...
Columns `pa_obs` and `pa_cobs` indicate the presence (1) or absence (0) of the specific taxon and taxa belonging to the same class.
...
taxa_examples <- app_reapp_global$appearing_taxa %>%
  tidylog::filter(in_BE == FALSE) %>%
  tidylog::filter(!taxonKey %in% app_reapp_global$reappearing_taxa$taxonKey) %>%
  tidylog::distinct(taxonKey, canonicalName) %>%
  tidylog::slice_head(n = 2)
taxa_examples %>%
  tidylog::left_join(df_ts_compact %>%
    tidylog::select(-starts_with("native")),
  by = "taxonKey"
  ) %>%
  tidylog::left_join(df_ts_compact_prot_areas %>%
    tidylog::select(-starts_with("native")) %>%
    tidylog::rename(
