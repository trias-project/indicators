...
Add columns `natura2000_Flanders`, `natura2000_Wallonia`, and `natura2000_Brussels` to the time series data to simplify the code while running the modelling for the natura2000 sites of each region separately:

```{r add_natura2000_regions}
df_ts <- df_ts%>%
  mutate(natura2000_Flanders=isFlanders & natura2000,
         natura2000_Wallonia=isWallonia & natura2000,
         natura2000_Brussels= isBrussels & natura2000)%>%
  rename(natura2000_Belgium=natura2000)
```
...
Preview from protected areas within each region:

```{r example_dec_rules_occs_protected_areas}
protected_areas <- c("natura2000_Flanders", "natura2000_Wallonia", "natura2000_Brussels","natura2000_Belgium")
purrr::map(
  protected_areas,
  function(x) {
    em_decision_rules_occs %>% 
