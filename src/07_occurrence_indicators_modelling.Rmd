...
We apply GAM for all taxa in Belgium, its regions and protected areas:

```{r apply_gam_to_all_taxa, message=FALSE, cache=TRUE}
# Define wrap function to apply GAM for every time series (BE, its regions and
# protected areas)
...
gam_occs$isBelgium$`7978544`$plot
gam_occs$issBelgium$`6157026`$plot
```


### Occupancy

Apply GAM for the observed area of occupancy (number of occupied cells):
...
```{r preview_gam_occupancy_Belgium, message=FALSE}
gam_occupancy$isBelgium$`7978544`$plot
gam_occupancy$isBelgium$`6157026`$plot
```
...
# Define function to save the results for Belgium, its regions and protected areas
save_decision_rules_output <- function(df,
                                       region,
                                       indicator) {
  readr::write_tsv(
    df,
...
purrr::iwalk(em_decision_rules_occs,
            save_decision_rules_output,
            indicator = "occs")

# Apply function to save decision rules outputs applied to occupancy
purrr::iwalk(em_decision_rules_occupancy,
            save_decision_rules_output,
            indicator = "occupancy")
```
