...
em_gam_list <- pmap(combinations, function(region, indicator) {
  ...
  readr::read_tsv(
        ...
        na = "") %>%
      dplyr::mutate(indicator =  paste(indicator, region, sep= "_"),
                      model = "GAM")
})
...
em_decision_rules_list <- pmap(combinations, function(region, indicator) {
  ...
  readr::read_tsv(
        ...
        na = "") %>%
    dplyr::select(-c(taxonName, kingdom)) %>%
    dplyr::mutate(
...
## Species names and kingdom

we read taxonomic information from the checklist taxon core:
...
taxon_core <- readr::read_tsv(
  ...
  dplyr::distinct(
    ...
    taxonName = acceptedName,
    class = acceptedClass,
    clasKey = acceptedClassKey,
    kingdom = acceptedKingdom)
```

These names will be added later to ranking for better readability.
...
spread_em_dfs <- function(em_list) {
  ...
  em_df <-
    ...
    dplyr::select(-c(dplyr::starts_with("growth"),
    ...
    dplyr::mutate(em = ifelse(is.na(GAM), decision_rules, GAM),
    ...
    dplyr::left_join(taxon_core, by = dplyr::join_by(taxonKey))
  em_df
}
```
...
em_df_be <-
  em_df_be %>%
  dplyr::left_join(em_gam_list$gam_occs_Belgium %>%
    dplyr::group_by(taxonKey) %>%
    dplyr::summarize(mean_growth = mean(growth, na.rm = TRUE))) %>%
  dplyr::relocate(.data$mean_growth, .before = .data$taxonName)
```
...
```{r preview_em_df_be}
em_df_be %>%
  dplyr::filter(!is.na(mean_growth)) %>%
...
em_df_regions <-
  em_df_regions %>% purrr::imap(
    function(df, region) {
      ...
      df %>%
        dplyr::left_join(em_gam_list[[occs_column]] %>%
          dplyr::group_by(taxonKey) %>%
          dplyr::summarize(mean_growth = mean(growth, na.rm = TRUE))) %>%
        dplyr::relocate(.data$mean_growth, .before = .data$taxonName)
    }
...
em_df_regions %>%
  purrr::map(function(df) {
    df %>%
      dplyr::filter(!is.na(mean_growth)) %>% dplyr::slice_head(n = 100)
})
```
...
2. emerging statuses in protected areas are more important than the ones defined over entire area
...
hierarchical_ranking <- function(years, indicators, regions, df) {
  ...
  ranking <- df %>%
    ...
    dplyr::select(
      taxonKey, taxonName, kingdom, class,
      cols_to_sort,
      dplyr::everything()
    )
  ranking
...
We apply the function to Belgium and its regions:
...
# Belgium
regions_be <- c("natura2000", "Belgium")
ranking_be <- hierarchical_ranking(years, indicators, regions_be, em_df_be)

# Flanders
regions_flanders <- c("FlandersNatura2000", "Flanders")
ranking_flanders <- hierarchical_ranking(years, indicators, regions_flanders, em_df_regions$Flanders)

# Wallonia
regions_wallonia <- c("WalloniaNatura2000", "Wallonia")
ranking_wallonia <- hierarchical_ranking(years, indicators, regions_wallonia, em_df_regions$Wallonia)

# Brussels
regions_brussels <- c("BrusselsNatura2000", "Brussels")
ranking_brussels <- hierarchical_ranking(years, indicators, regions_brussels, em_df_regions$Brussels)
rankings <- list(
  "Belgium" = ranking_be,
  "Flanders" = ranking_flanders,
  "Wallonia" = ranking_wallonia,
  "Brussels" = ranking_brussels
)
```

Results:

```{r results_hierarchical_ranking}
rankings
```

## Point strategy

We can also rank using a point strategy reflecting the same ranking strategy as before. To do so we apply gain factors to all emerging status scores using the number of observations in 2022 in the entire Belgium/region as reference (gain factor  = 1).

The following gain factors are applied:

| indicator | year | region | gain factor |
| --- | --- | --- | --- | 
occupancy | 2024 | protected areas | 3
observations | 2024 | protected areas | 2.5
occupancy | 2024 | Belgium/region | 2.5
observations | 2024 | Belgium/region | 2
occupancy | 2023 | protected areas | 2.5
observations | 2023 | protected areas | 2
occupancy | 2023 | Belgium/region | 2
observations | 2023 | Belgium/region | 1.5
occupancy | 2022 | protected areas | 2
observations | 2022 | protected areas | 1.5
occupancy | 2022 | Belgium/region | 1.5
observations | 2022 | Belgium/region | 1

```{r rank_by_em_pts_be}
ranking_pts_be <-
  em_df_be %>%
  dplyr::mutate(
    em_pts = year_2024_em_status_occupancy_natura2000 * 3 +
      year_2024_em_status_occs_natura2000 * 2.5 +
      year_2024_em_status_occupancy_Belgium * 2.5 +
      year_2024_em_status_occs_Belgium * 2 +
      year_2023_em_status_occupancy_natura2000 * 2.5 +
      year_2023_em_status_occs_natura2000 * 2 +
      year_2023_em_status_occupancy_Belgium * 2 +
      year_2023_em_status_occs_Belgium * 1.5 +
      year_2022_em_status_occupancy_natura2000 * 2 +
      year_2022_em_status_occs_natura2000 * 1.5 +
      year_2022_em_status_occupancy_Belgium * 1.5 +
  ...
  dplyr::select(
    taxonKey, taxonName, kingdom, class,
    em_pts, mean_growth,
    year_2024_em_status_occupancy_natura2000,
    year_2024_em_status_occs_natura2000,
    year_2024_em_status_occupancy_Belgium,
    year_2024_em_status_occs_Belgium,
    year_2023_em_status_occupancy_natura2000,
    year_2023_em_status_occs_natura2000,
    year_2023_em_status_occupancy_Belgium,
    year_2023_em_status_occs_Belgium,
...
ranking_pts_regions <- purrr::imap(
  ...
  function(em_df, region) {
    em_df %>%
      ...
      dplyr::rename_with(~ gsub(paste0("_",region), "", .x)) %>%
      dplyr::mutate(
        em_pts = 
          year_2024_em_status_occupancyNatura2000 * 3 +
          year_2024_em_status_occsNatura2000 * 2.5 +
          year_2024_em_status_occupancy * 2.5 +
          year_2024_em_status_occs * 2 +
          year_2023_em_status_occupancyNatura2000 * 2.5 +
          year_2023_em_status_occsNatura2000 * 2 +
          year_2023_em_status_occupancy * 2 +
          year_2023_em_status_occs * 1.5 +
          year_2022_em_status_occupancyNatura2000 * 2 +
          year_2022_em_status_occsNatura2000 * 1.5 +
          year_2022_em_status_occupancy * 1.5 +
      ...
      dplyr::select(
        taxonKey, taxonName, kingdom, class,
        em_pts, mean_growth,
        year_2024_em_status_occupancyNatura2000,
        year_2024_em_status_occsNatura2000,
        year_2024_em_status_occupancy,
        year_2024_em_status_occs,
        year_2023_em_status_occupancyNatura2000,
        year_2023_em_status_occsNatura2000,
        year_2023_em_status_occupancy,
        year_2023_em_status_occs,
        year_2022_em_status_occupancyNatura2000,
        year_2022_em_status_occsNatura2000,
        year_2022_em_status_occupancy,
        year_2022_em_status_occs,
        dplyr::everything()
      ) %>%
      # Add region back to the column names starting with `year`
      dplyr::rename_with(~ paste0(.x, "_", region), starts_with("year"))
  }
)
ranking_pts_regions
```
...
The ranking for Belgiua/regions based on hierarchical strategy are saved in `data/output/ranking_emerging_status_hierarchical_strategy_*.tsv` where * is the region or `"Belgium"`:
