...
and the final emerging status for protected areas within specific regions, `em_df_protected_regions`:

```{r save_em_df_protected_regions}
purrr::iwalk(
  em_df_protected_regions,
  function(df, region) {
    readr::write_tsv(df, 
...
The ranking for protected areas within each region based on hierarchical strategy are saved in `data/output/ranking_emerging_status_hierarchical_strategy_natura2000_*.tsv` where * is the region:

```{r save_ranking_protected_ regions}
purrr::iwalk(
  ranking_protected_regions,
  function(df, region) {
    readr::write_tsv(df,
...
Ranking for the protected areas within each region based on points strategy are saved in `data/output/ranking_emerging_status_points_strategy_natura2000_*.tsv` where * is the region:

```{r save_ranking_pts_protected_regions}
purrr::iwalk(
  ranking_pts_protected_regions,
  function(df, region) {
    readr::write_tsv(df,
