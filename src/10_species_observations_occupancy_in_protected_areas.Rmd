...
Read the occurrence cubes at species level for European countries as published on Zenodo (https://doi.org/10.5281/zenodo.10074895). It can take a long time:

```{r read_cube_from_zenodo}
if (!"be_species_cube.csv" %in%
  list.files(here::here("data", "input"))) {
  inborutils::download_zenodo(
    doi = "10.5281/zenodo.10074895",
...
be_species_cube <- read_csv(
  file = here::here("data", "input", "be_species_cube.csv"),
  na = ""
)
be_species_info <- read_csv(
  file = here::here("data", "input", "be_species_info.csv"),
  na = "") %>%
...
Read the occurrence cubes of alien taxa as published on Zenodo (https://doi.org/10.5281/zenodo.10058400). It is based on GBIF occurrence data of taxa published in the [Global Register of Introduced and Invasive Species - Belgium](https://www.gbif.org/dataset/6d9e952f-948c-4483-9807-575348147c7e), shortly called the [unified checklist](https://github.com/trias-project/unified-checklist). It can take a long time:

```{r read_cube_from_zenodo_alien}
if (!"be_alientaxa_cube.csv" %in%
  list.files(here::here("data", "input"))) {
  inborutils::download_zenodo(
    doi = "10.5281/zenodo.10058400",
    path = here::here("data", "input"), 
  )
}
```
...
be_alientaxa_info <- read_csv(
  file = here::here("data", "input", "be_alientaxa_info.csv"),
  na = "") %>%
...
infraspecific_alien <-
  be_alientaxa_info %>%
  tidylog::distinct(rank, taxonKey) %>%
  tidylog::filter(rank %in% c("SUBSPECIES",
                              "VARIETY",
                              "FORM"))
infraspecific_alien <-
  purrr::map_dfr(
...
infraspecific_alien_status_species <-
  readr::read_tsv(here::here("reference",
...
occs_prot_areas_infraspecific <-
  occs_prot_areas_infraspecific %>%
  tidylog::left_join(infraspecific_alien %>%
