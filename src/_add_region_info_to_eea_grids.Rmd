...
We read the spatial data containing information about Belgian administrative regions:
...
Transform the Belgian regions to the Coordinate Reference System (CRS) of the grids:
...
purrr::map(eea_grids, function(x) {
  x %>% 
    dplyr::as_tibble() %>%
    dplyr::group_by(isFlanders, isWallonia, isBrussels) %>% 
    dplyr::count()
  }
)
```
